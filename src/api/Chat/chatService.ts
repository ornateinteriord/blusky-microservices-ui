...
export const useSearchMember = () => {
  return useMutation({
    mutationFn: async (mobileNumber: string) => {
      const response = await api.get(`/chat/search?mobileNumber=${mobileNumber}`);
      return response.data;
    },
  });
};
...
export const useSendMessage = () => {
  return useMutation({
    mutationFn: async (messageData: { roomId: string; text?: string; imageUrl?: string; messageType?: string }) => {
      const response = await api.post("/chat/message/send", messageData);
      return response.data;
    },
  });
};
...
export const useMarkAsRead = () => {
  ...
  return useMutation({
    mutationFn: async (roomId: string) => {
      const response = await api.patch(`/chat/mark-read/${roomId}`);
      return response.data;
    },
    onSuccess: () => {
...
export const useDeleteMessage = () => {
  return useMutation({
    mutationFn: async (messageId: string) => {
      const response = await api.remove(`/chat/message/${messageId}`);
      return response.data;
    },
