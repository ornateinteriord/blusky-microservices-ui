...
import { DASHBOARD_CUTSOM_STYLE, getFixedDepositWalletColumns,  } from '../../../utils/DataTableColumnsProvider';
import TokenService from "../../../api/token/tokenService";
import { useGetWalletOverview } from '../../../api/Memeber';

const FixedDepositWallet = () => {
    ...
    >
      <CardContent sx={{ padding: isMobile ? "12px" : "24px" }}>
        <Grid container spacing={3} sx={{ mb: 3 }}>
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              >
                ${Number(walletData?.fixedDepositBalance || 0).toFixed(4)}
              </Typography>
            </Box>
          </Grid>
        </Grid>
        ...
        <div style={{ marginBottom: "1rem", color: "#000", fontWeight: "bold", fontSize: "1.25rem"     }}>Transaction History</div>
          {walletData?.fixedDepositTransactions && walletData.fixedDepositTransactions.length > 0 ? (
              <DataTable
                columns={getFixedDepositWalletColumns()}
                data={walletData.fixedDepositTransactions}
                pagination
...
export default FixedDepositWallet;
