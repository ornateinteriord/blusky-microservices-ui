  ...
  const quickAccessGroups = [
    {
      ...
      items: [
        ...
        { label: "Property Listings", icon: <InventoryIcon />, onClick: () => navigate("/user/new-subscription"), route: "", color: "#0284C7" },
        { label: "My Properties", icon: <ReceiptLongIcon />, onClick: () => navigate("/user/my-subscriptions"), route: "", color: "#f59e0b" },
      ]
    },
    {
      title: "BENEFITS",
      items: [
        // { label: "ROI Benefits", icon: <ShowChartIcon />, route: "/user/earnings/roi-benefits", color: "#0284C7" },
        ...
        // { label: "Level Bonus", icon: <AccountTreeIcon />, route: "/user/earnings/level-benefits", color: '#0284C7' },
        { label: "Transactions", icon: <AttachMoneyIcon />, route: "/user/transactions", color: '#0284C7' },
      ]
    },
    {
      title: "TEAM & TOOLS",
  ...
  return (
    <Box sx={{
      ...
      <Paper elevation={0} sx={{ 
        ...
        <Box>
          <Typography sx={{ color: '#475569', fontSize: '0.70rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', mb: 0.5 }}>
            User ID
          </Typography>
          <Typography sx={{ color: '#0F172A', fontSize: { xs: '1.1rem', sm: '1.3rem' }, fontWeight: 900 }}>
            {memberDetails?.Member_id || memberId}
          </Typography>
          <Typography sx={{ color: '#0284C7', fontSize: { xs: '0.85rem', sm: '1rem' }, fontWeight: 700, mt: 0.2 }}>
        ...
        <Box sx={{ 
          ...
          <Box onClick={() => navigate('/user/top-up-wallet')} sx={{ display: 'flex', flexDirection: 'column', p: { xs: 2, sm: 3 }, borderRadius: '20px', bgcolor: '#F8FAFC', border: '1px solid #E2E8F0', transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.02)', bgcolor: '#F1F5F9' }, cursor: 'pointer' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', mb: 2 }}>
              ...
              <Typography sx={{ color: '#0F172A', fontWeight: 900, fontSize: { xs: '1.3rem', sm: '1.6rem' }, letterSpacing: '1px', m: 0 }}>{Number(walletOverview?.topUpBalance || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Typography>
            </Box>
            <Box sx={{ width: '100%' }}>
              <Typography sx={{ color: '#1E293B', fontWeight: 800, fontSize: { xs: '1rem', sm: '1.2rem' }, textTransform: 'uppercase', mb: 0.5 }}>Top Up</Typography>
        ...
        <Box sx={{
          ...
          <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 3 }}>
                {quickAccessGroups.flatMap(group => group.items).map((item: any, i: number) => (
                  <Box key={i} onClick={() => item.onClick ? item.onClick() : navigate(item.route)} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5, cursor: 'pointer' }}>
                    ...
                    <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.7rem', textAlign: 'center', color: '#1E293B' }}>
                      {item.label}
                    </Typography>
                  </Box>
                ))}
            ...
            <Box sx={{ mt: 2, display: { xs: 'block', md: 'none' } }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 900, color: '#0F172A', mb: 2 }}>BROKERAGE PERFORMANCE</Typography>
              <Paper elevation={0} sx={{ p: 3, borderRadius: '20px', bgcolor: '#F1F5F9', border: '1px solid #E2E8F0' }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                  <Box 
                    onClick={() => navigate('/user/team')}
                    sx={{ textAlign: 'center', cursor: 'pointer', '&:hover': { opacity: 0.8 }, display: 'flex', flexDirection: 'column', alignItems: 'center' }}
                  >
                    <GroupsIcon sx={{ fontSize: 32, color: '#0284C7', mb: 1 }} />
                    <Typography variant="caption" sx={{ color: '#64748B', fontWeight: 700 }}>My Agents</Typography>
                    <Typography variant="h4" sx={{ fontWeight: 900, color: '#0F172A' }}>{Math.max(memberDetails?.registration_stats?.total || 0, memberDetails?.total_team || 0)}</Typography>
                  </Box>
                  <Box 
                    onClick={() => navigate('/user/team/direct')}
                    sx={{ textAlign: 'center', borderLeft: '1px solid #E2E8F0', cursor: 'pointer', '&:hover': { opacity: 0.8 }, display: 'flex', flexDirection: 'column', alignItems: 'center' }}
                  >
                    ...
                    <Typography variant="caption" sx={{ color: '#64748B', fontWeight: 700 }}>Direct Clients</Typography>
                    <Typography variant="h4" sx={{ fontWeight: 900, color: '#0F172A' }}>{Math.max(memberDetails?.registration_stats?.direct || 0, memberDetails?.direct_referrals?.length || 0)}</Typography>
                  </Box>
                </Box>
              </Paper>
            </Box>
          ...
          <Box sx={{ width: { xs: '100%', xl: '420px' }, display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Paper elevation={0} sx={{ p: 4, borderRadius: '32px', bgcolor: '#F1F5F9', border: '1px solid #E2E8F0', boxShadow: '0 10px 40px rgba(0,0,0,0.05)', mt: 1 }}>
              <Stack spacing={4}>
